...
public class PlaceHierarchiesOnAxesImplIT extends AbstractTransformTestCase<PlaceHierarchiesOnAxes> {
	...
	public void testFindVisibleHierarchies() {
		...
		List<Hierarchy> columnHierarhies = transform.findVisibleHierarchies(Axis.COLUMNS);
		List<Hierarchy> rowHierarhies = transform.findVisibleHierarchies(Axis.ROWS);

		assertThat("Hierarchy list on the column axis should not be null", columnHierarhies, is(notNullValue()));
		assertThat("Number of hierarchy on the column axis should be 1", columnHierarhies.size(), is(equalTo(1)));

		assertThat("Wrong name for the first hierarchy on the column axis", columnHierarhies.get(0).getName(),
				is(equalTo("Measures")));

		assertThat("Hierarchy list on the row axis should not be null", rowHierarhies, is(notNullValue()));
		assertThat("Number of hierarchies on the column axis should be 2", rowHierarhies.size(), is(equalTo(2)));

		assertThat("Wrong name for the first hierarchy on the row axis", rowHierarhies.get(0).getName(),
				is(equalTo("Promotion Media")));
		assertThat("Wrong name for the seconde hierarchy on the row axis", rowHierarhies.get(1).getName(),
				is(equalTo("Product")));
	}
}
